/**
 * One-off runner for the Apollo press-release scraper.
 *
 * Usage:
 *   pnpm tsx --env-file=.env.local scripts/scrape-apollo.ts [daysBack=60] [maxKept=20]
 */

import { createSupabaseAdminClient } from "@/lib/supabase/admin";
import { apolloConfig } from "@/lib/scrapers/apollo";
import {
  matchedKeywords,
...
async function main() {
  ...
  const r = await scrapeGpPressReleases(supabase, apolloConfig, {
    daysBack,
    maxKept,
    maxProbed: 50,
  });

  console.log(`\n== Apollo scrape summary ==`);
