...
export default function PensionPageError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error("[/pensions/[slug]] route error:", error);
  }, [error]);
    ...
    <div className="card-surface py-16 px-6 flex flex-col items-center justify-center gap-4 text-center">
      ...
      <div>
        <div className="text-[14px] font-semibold text-ink">
          Pension page failed to load
        </div>
        <div className="mt-1 text-[12.5px] text-ink-muted max-w-md leading-relaxed">
          {error.message || "An unexpected error occurred fetching this plan."}
          {error.digest ? (
